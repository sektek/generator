...
import { resolveConfigDefaults } from './resolve-config-defaults.js';
...
describe('resolveConfigDefaults', function () {
  ...
  beforeEach(function () {
    root = mkdtempSync(join(tmpdir(), 'sektek-generator-config-'));
    home = mkdtempSync(join(tmpdir(), 'sektek-generator-config-home-'));
  });
